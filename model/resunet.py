...
def get_normalization(normtype: str, num_channels: int, dim: int = 2):
...
def conv3(in_channels, out_channels, padding_mode='zeros',kernel_size=3, stride=1,
...
def upconv2(in_channels, out_channels, mode='transpose', planar=False, dim=2, normalization='batchnorm', activation='relu'):
    ...
    elif 'pixel_shuffle' in mode:
        #PS: (∗,C×(r^2),H,W) -> (∗,C,H×r,W×r) with r=2 (∗,C×4,H,W) -> (∗,C,H×2,W×2) (need to increase channels)
        #psh_scale = 2
        return PixShBlock(in_channels, out_channels, normalization=normalization, kernel_size=3, stride=1, dim=2,planar=False, padding_mode='zeros',padding=1)
...
def conv1(in_channels, out_channels, dim=2):
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = conv_class(in_channels, out_channels, kernel_size=kernel_size, padding=padding,padding_mode=padding_mode)
        self.norm1 = get_normalization(normalization, self.out_channels, dim=dim)
        self.act1 = get_activation(activation)
        self.conv2 = conv_class(out_channels, out_channels, kernel_size=kernel_size, padding=padding, padding_mode=padding_mode)
...
#NOTE changed res block position! (moved to after activation)
...
class DownBlock(nn.Module):
    def __init__(
        ...
        convs = [ConvBlock(
            ...
            normalization=normalization, conv_mode=conv_mode, padding_mode=padding_mode,
        ...
        for _ in range(res_blocks - 1):  # -1 because we have already added one above
            convs.append(ConvBlock(
                ...
                normalization=normalization, conv_mode=conv_mode, residual=enable_residual,padding_mode=padding_mode
            ))
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 ...
                 activation='relu', normalization=None, full_norm=True, dim=2, conv_mode='same', padding_mode="zeros",
        ...
        self.upconv = upconv2(self.in_channels, self.out_channels, mode=self.up_mode, planar=planar, dim=dim,normalization=self.normalization, activation=self.activation)
...
class PixShBlock(nn.Module):
    """
    Pixel Shuffle Upsampling
    """
    def __init__(self, in_channels, out_channels, normalization="batchnorm", kernel_size=3, stride=1, dim=2,planar=False, padding_mode='zeros',padding=1):
        super().__init__()
        ...
        self.pixsh = nn.PixelShuffle(self.psh_scale)

        self.conv1 = conv1(in_channels=in_channels//(self.psh_scale)**2,out_channels=in_channels//self.psh_scale,dim=dim)

        self.norm1 = get_normalization(normalization, num_channels=in_channels//self.psh_scale, dim=dim)

        self.conv = get_conv(dim=dim)(in_channels=in_channels//self.psh_scale,
    ...
    def forward(self, x):
        return self.conv(self.norm1(self.conv1(self.pixsh(x))))   
...
class ResizeConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, planar=False, dim=2,
                 upsampling_mode='nearest'):
        super().__init__()
...
class GridAttention(nn.Module):
    ...
    def __init__(self, in_channels, gating_channels, inter_channels=None, dim=2, sub_sample_factor=2):
        super().__init__()
    ...
    Args:
        ...
        up_mode: Upsampling method in the decoder pathway.
            ...
            - 'resizeconv_nearest': Use resize-convolution with nearest-
              ...
              https://distill.pub/2016/deconveckercheckerboard/
        ...
        conv_mode: Padding mode of convolutions. Choices:
            ...
            final_activation: use nonlinear layer as the last layer ('lin' means using nn.Identity placeholder for it)
    """
    def __init__(
            self,
            in_channels: int = 4,
            ...
            dim: int = 2,
            conv_mode: str = 'same',
            final_activation: Union[str, nn.Module] = 'lin'
    ):
        super().__init__()
        ...
        if up_mode in ('transpose', 'upsample', 'resizeconv_nearest', 'resizeconv_linear',
                       'resizeconv_nearest1', 'resizeconv_linear1', 'pixel_shuffle'):
            self.up_mode = up_mode
        else:
        ...
        for i in range(n_blocks):
            ...
            outs = self.start_filts * (ch_scale**i)
        ...
        self.act_final = get_activation(final_activation)
